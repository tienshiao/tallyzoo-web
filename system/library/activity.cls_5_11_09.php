...
class activity
{
	...
	function getUsersActivityWithCount($userId,$pageLimit="",$filer="")
	{
	...
	{
		$sql = "SELECT activity_id FROM graphs_activities g INNER JOIN activities a ON  a.id=g.activity_id WHERE g.graph_id=\"$graphId\" AND a.deleted=\"0\"";
		$ROW = $this->db->select($sql);
		$ids = "";
		foreach($ROW as $i)
		{
	...
	{
		 $sql = "SELECT a.name,a.color,g.activity_id FROM graphs_activities as g INNER JOIN activities as a ON a.id=g.activity_id WHERE g.graph_id=\"$graphId\" AND a.deleted=\"0\"";
		$ROW = $this->db->select($sql);
		return $ROW;
	}
...
function InsertIntoDashboard($dashboardId,$userId,$activityId,$graphid=''){
...
function getDashboardActivityGraph($actId)
	{
	 $sql = "SELECT a.*,g.hidden,g.graph_type,g.dataOption,d.id as d_id FROM activities as a inner join graphs g on g.id=a.graph_id,dashboard AS d WHERE (a.user_id=\"$actId\" AND d.activity_id = a.id) group by g.id";
		$ROW = $this->db->select($sql);
	...
	function removeFromDashBoard($d_id)
	{
