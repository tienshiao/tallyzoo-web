...
img_Del.src="images/calendar/btn_del_small_gif";
img_Close.src="images/calendar/btn_close_small_gif";
...
function spiffyCalManager() {
	...
	this.matchedFormat="";
	this.DefBtnImgPath='images/calendar/';
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	switch (intBtnMode) {
		case 0 :
			strBtnImg=strDefBtnImgPath+'cal.gif';
			img_DateBtn_UP.src=strDefBtnImgPath+'cal.gif';
			img_DateBtn_OVER.src=strDefBtnImgPath+'cal.gif';
			img_DateBtn_DOWN.src=strDefBtnImgPath+'cal.gif';
			img_DateBtn_DISABLED.src=strDefBtnImgPath+'cal.gif';
			strBtnW = '19';
			strBtnH = '15';
			break;
		case 1 :
...
// set year range for the calendar control
	this.minYearChoice=dteToday.getFullYear()-50;																		//dteToday.getFullYear();
	this.maxYearChoice=	dteToday.getFullYear()+50;																						//dteToday.getFullYear() + 5;
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		strOutput += '<div class="cal_inside">'
		// special case for form not to be inside table in Netscape 6
		
			strOutput += strTemp + '<table width="100%" border="0" class="cal-Table" cellspacing="0" cellpadding="0"><tr>';
		

		if (!(bViewOnly)) {
			strOutput += '<td class="cal-HeadCell" align="center" width="100%"><a href="javascript:'+this.varName+'.clearDay();"><img title="Click to clear off already selected date." name="calbtn1" src="'+strDefBtnImgPath+'btn_del_small.gif" border="0" width="12" height="10"></a>&nbsp;&nbsp;&nbsp;&nbsp;<SELECT class="cal-ComboBox" NAME="cboMonth" onChange="'+this.varName+'.changeMonth();">';
			...
			strOutput += '<\/SELECT>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:'+this.varName+'.hide();"><img title="Click to close the calendar." name="calbtn2" src="'+strDefBtnImgPath+'btn_close_small.gif" border="0" width="12" height="10"></a><\/td><\/tr><tr><td width="100%" align="center">';
		}
		else {
		...
		strOutput +="<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\" class=\"tbl_time\"><tr><td width='40'>Time:<\/td><td>" + getClockTimeText(); + "<\/td><\/tr><\/table>";
		return strOutput;
	}	
	...
	function writeControl() {
		...
		strHold='<input class="fld_txt wd_115" tabindex="" type="text"  readonly name="' + this.textBoxName + '"' + strTemp + 'size="10"  maxlength="10" value="' + this.getSelectedDate() + '"  onClick="alert(\'Select Calendar\');">';
		if (!scIE) {
...
//		strHold+=this.varName+'.show();return false;">';
		...
		strHold+='<img tabindex="" title="Click to open the calendar control. " align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg +'" width="'+ strBtnW +'" height="'+ strBtnH +'" ></a>';
...
// put these next 2 lines in when the tiny cal btns seem to randomly disappear		
	...
	function changeDay(intWhatDay) {
		...
		dteCur.setHours(0);dteCur.setMinutes(0);dteCur.setSeconds(0);dteCur.setMilliseconds(0);
		strTime = " " + document.getElementById("txtTime").value + " " + document.getElementById("txtAmPm").value;
		var dateval =eval(document.forms["spiffyCal"].cboMonth.options[document.forms["spiffyCal"].cboMonth.selectedIndex].value)+1+'-'+intWhatDay+'-'+document.forms["spiffyCal"].cboYear.options[document.forms["spiffyCal"].cboYear.selectedIndex].value//calMgr.scFormatDate(dteCur,this.dateFormat);
		var arr = dateval.split("-");
		var dateval = arr[1] + "/" + arr[0] + "/" + arr[2];
		this.textBox().value= dateval + strTime;//calMgr.scFormatDate(dteCur,this.dateFormat);
		this.hide();
		if (this.JStoRunOnSelect!=null)
			eval(unescape(this.JStoRunOnSelect)); 

		eval('document.'+this.formName+'.'+this.textBoxName).className = "fld_txt wd_115";
...
function getClockTimeText()
{
   ...
   str = "<input type='text' class=\"fld_txt_cal\" style=\"width:60px;\" id='txtTime' value=\"" + timeString + "\">&nbsp;<input class=\"fld_txt_cal\" style=\"width:30px;\" type='text' id='txtAmPm' value=\"" +ap + "\">";
   return str;
}